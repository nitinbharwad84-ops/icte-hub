...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'view_college') {
      ...
      if (fetchErr) console.error('Tracking fetch error (view_college):', fetchErr);
      ...
      if (upsertErr) console.error('Tracking upsert error (view_college):', upsertErr);
      ...
      if (fetchErr) console.error('Tracking fetch error (filter_change):', fetchErr);
      ...
      if (upsertErr) console.error('Tracking upsert error (filter_change):', upsertErr);
    } else if (action === 'converted') {
      const { error: upsertErr } = await supabase.from('visitors').upsert(
        { session_id, converted_to_lead_id: payload.lead_id, last_seen_at: new Date().toISOString() },
        { onConflict: 'session_id' }
      );
      if (upsertErr) console.error('Tracking upsert error (converted):', upsertErr);
    } else if (action === 'page_view') {
      const { error: upsertErr } = await supabase.from('visitors').upsert(
        { session_id, last_seen_at: new Date().toISOString() },
        { onConflict: 'session_id' }
      );
      if (upsertErr) console.error('Tracking upsert error (page_view):', upsertErr);
    }
